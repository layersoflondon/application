namespace :lol do
  ...
  task generate_historical_maps: :environment do
    # Load seed file
    layers = YAML.load_file(File.join(Rails.root, 'db','layer_seeds.yml')).collect(&:with_indifferent_access)
    layers.each do |layer|
      ...
      i.file.attach(io: File.open(File.join(Rails.root,"db",layer[:image])), filename: layer[:image])
      l.image = i
      l.save
    end
  end
