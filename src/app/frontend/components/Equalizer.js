...
export default class Equalizer extends Component {
    ...
    updateChildrenHeights() {
        ...
        if (node !== undefined) {
            let children          = this.props.nodes(this, node)
            if( this.props.selector ) {
                children = Array.from(children).map((child) => child.querySelector(this.props.selector));
            }
    ...
    render() {
        ...
        return (
            <div ref={node => this.rootNode = node} onLoad={this.handleResize} >
                {children}
            </div>
        )
    }
